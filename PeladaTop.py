...
# ------------------ Tela 2 ------------------
    ...
    with col1:
        ...
        for i, jogador in enumerate(st.session_state.time_a):
            c = st.columns([4, 1])  # nome + botão na mesma linha
            c[0].markdown(f"<span style='font-size:16px'>{jogador}</span>", unsafe_allow_html=True)
            if c[1].button("➖", key=f"rm_timea_{i}"):
    ...
    with col2:
        ...
        for i, jogador in enumerate(st.session_state.time_b):
            c = st.columns([4, 1])  # nome + botão na mesma linha
            c[0].markdown(f"<span style='font-size:16px'>{jogador}</span>", unsafe_allow_html=True)
            if c[1].button("➖", key=f"rm_timeb_{i}"):
                st.session_state.confirm_remove = jogador
    ...
    # Adicionar novos jogadores durante a partida (indentação corrigida)
    ...
    # Fila com nome + botão na mesma linha
    ...
    for idx, bloco in enumerate(blocos):
        ...
        if bloco:
            for j, jogador in enumerate(bloco):
                c = st.columns([4, 1])
                c[0].markdown(f"<span style='font-size:16px'>{jogador}</span>", unsafe_allow_html=True)
                if c[1].button("➖", key=f"rmfila_{idx}_{j}"):
